...
extern "C" void ClearSegList(float xi, float yi, float zi)
{
    seglistsize = 0;
    seglist[0].p1[0] = xi;
    seglist[0].p1[1] = yi;
                              ...
                              float accel )
{
    ...
    if (freq == P_120HZ)    //set fast path mode if using 120 Hz path
    {
        ...
        ServoSetIoCtrl(x, (byte)(ioctrl | FAST_PATH));
        ioctrl = ServoGetIoCtrl(y);
        ServoSetIoCtrl(y, (byte)(ioctrl | FAST_PATH));
        if (z)
        {
            ioctrl = ServoGetIoCtrl(z);
            ServoSetIoCtrl(z, (byte)(ioctrl | FAST_PATH));
        }
    }
    else    //clear fast path bit if using slower modes
    {
        ioctrl = ServoGetIoCtrl(x);
        ServoSetIoCtrl(x, (byte)(ioctrl & ~((byte)(FAST_PATH)) ));
        ioctrl = ServoGetIoCtrl(y);
        ServoSetIoCtrl(y, (byte)(ioctrl & ~((byte)(FAST_PATH)) ));
        if (z)
        {
            ioctrl = ServoGetIoCtrl(z);
            ServoSetIoCtrl(z, (byte)(ioctrl & ~((byte)(FAST_PATH)) ));
        }
    }
    ...
    {
        qDebug() << "Required status items have not been set";
        return(-1);
    }
...
int GetNextPathpoint(int *xp, int *yp, int *zp)
{
    ...
    {
        *xp = (int)( (seglist[seglistsize-1].p2[0] + xoff) * UTOCX );
        *yp = (int)( (seglist[seglistsize-1].p2[1] + yoff) * UTOCY );
        *zp = (int)( (seglist[seglistsize-1].p2[2] + zoff) * UTOCZ );
    }
    else   //find the point within the current segment
    {
        ...
        *xp = (int)( (p[0] + xoff) * UTOCX );
        *yp = (int)( (p[1] + yoff) * UTOCY );
        *zp = (int)( (p[2] + zoff) * UTOCZ );
    }
...
{
    int xp[7], yp[7], zp[7];  //set of up to 7 pathpoints
    int pcount;
