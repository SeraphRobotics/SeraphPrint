...
#include <QLibrary>
QLibrary pathlib("PATHLIB4");
typedef void (*SetTangentTolerancePrototype)(float);
typedef void (*ClearSegListPrototype)(float,float,float);
typedef int (*AddLineSegPrototype)(float,float,float);
typedef int (*AddArcSegPrototype)(float,float,float);
typedef void (*SetFeedratePrototype)(float);
typedef void (*SetOriginPrototype)(float,float,float);
typedef int (*SetPathParamsPrototype)(int,int,int,int,int,int,int,float,float,float,float);
typedef int (*SetPathParams2Prototype)(int,int,int,int,int,int,int,float,float,float,float);
typedef float (*InitPathPrototype)(void);
typedef int (*AddPathPointsPrototype)(void);


SetTangentTolerancePrototype SetTangentTolerance  =(SetTangentTolerancePrototype) pathlib.resolve("SetTangentTolerance");

ClearSegListPrototype ClearSegList = (ClearSegListPrototype) pathlib.resolve("ClearSegList");

AddLineSegPrototype AddLineSeg = (AddLineSegPrototype) pathlib.resolve("AddLineSeg");

AddArcSegPrototype AddArcSeg = (AddArcSegPrototype) pathlib.resolve("AddArcSeg");

SetFeedratePrototype SetFeedrate = (SetFeedratePrototype) pathlib.resolve("SetFeedrate");

SetOriginPrototype SetOrigin = (SetOriginPrototype) pathlib.resolve("SetOrigin");

SetPathParamsPrototype SetPathParams = (SetPathParamsPrototype) pathlib.resolve("SetPathParams");

SetPathParams2Prototype SetPathParams2 = (SetPathParams2Prototype) pathlib.resolve("SetPathParams2");

InitPathPrototype InitPath = (InitPathPrototype) pathlib.resolve("InitPath");

AddPathPointsPrototype AddPathPoints = (AddPathPointsPrototype) pathlib.resolve("AddPathPoints");
