...
void test(QString filename,NPath h){
    ...
    // This produced a series of CSV documents that explore NPath and HzPath in relative and absolute versions
...
void printMaterial(Material m){
    printf("\n Materail:%s",m.name.toStdString().c_str());
    printf("\n\tQ:%f",m.Q);
    QStringList keys= m.property.keys();
    QStringListIterator i(keys);
    while(i.hasNext()){
        QString temp =i.next();
        printf("\n\t%s:%s",temp,m.property[temp]);
    }
}
