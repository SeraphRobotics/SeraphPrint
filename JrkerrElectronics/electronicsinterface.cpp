...
ElectronicsInterface::ElectronicsInterface():initialized_(false)
{
}
...
QString ElectronicsInterface::loadSettings(QDomNode node){
    ...
    for(int i=0; i<electronicsChildren.length();i++){
        ...
        if("controller"==echild.nodeName().toLower()){
            ...
            for(int j=0; j<controllerChildren.length();j++){
...
void ElectronicsInterface::createMotor(QDomNode sourceDomNode){
        ...
        //// All of the XML parsing to make the motor
        {
        QDomNodeList actuatorChildren = sourceDomNode.childNodes();
        for(int i=0; i<actuatorChildren.length();i++){
