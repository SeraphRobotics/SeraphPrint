...
//---------------------------------------------------------------------------//
bool IoGetStat(byte addr,QString* error_string)
{
...
if (numrcvd != numbytes)
    ...
    ss<<"IoGetStat failed to read chars";
    return false;
    }
...
if (cksum != inbuf[numbytes-1]){
    QTextStream ss(error_string,QIODevice::WriteOnly);
    ss<<"IoGetStat: line 87: checksum error";
    return false;
    }
...
if (mod[addr].stat & CKSUM_ERROR)
	{
    QTextStream ss(error_string,QIODevice::WriteOnly);
    ss<<"IoGetStat: line 96: Command checksum error!";
    return false;
    }
...
//---------------------------------------------------------------------------//
bool IoSetOutBit(byte addr, int bitnum,QString* error_string)
{
IOMOD * p;
...
return NmcSendCmd(addr, SET_OUTPUT, (char *)(&(p->outbits)), 2, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool IoClrOutBit(byte addr, int bitnum,QString* error_string)
{
IOMOD * p;
...
//---------------------------------------------------------------------------//
bool IoBitDirOut(byte addr, int bitnum,QString* error_string)
{
IOMOD * p;
...
return NmcSendCmd(addr, SET_IO_DIR, (char *)(&(p->bitdir)), 2, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool IoBitDirIn(byte addr, int bitnum, QString* error_string)
{
IOMOD * p;
...
//---------------------------------------------------------------------------
bool IoSetPWMVal(byte addr, byte pwm1, byte pwm2, QString* error_string)
{
IOMOD * p;
char cmdstr[4];
...
return NmcSendCmd(addr, SET_PWM, cmdstr, 2, addr, error_string);
}
...
//---------------------------------------------------------------------------//
bool IoSetSynchOutput(byte addr, short int outbits, byte pwm1, byte pwm2,QString* error_string)
{
IOMOD * p;
char cmdstr[5];
...
cmdstr[2] = pwm1;
cmdstr[3] = pwm2;
return NmcSendCmd(addr, SET_SYNCH_OUT, cmdstr, 4, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool IoSetTimerMode(byte addr, byte tmrmode,QString* error_string)
{
IOMOD * p;
char cmdstr[2];
...
return NmcSendCmd(addr, SET_TMR_MODE, cmdstr, 1, addr,error_string);
}
