...
//---------------------------------------------------------------------------//
bool ServoGetStat(byte addr,QString* error_string)
{
    ...
    numrcvd = SioGetChars(ComPort, (char *)inbuf, numbytes,error_string);

    //Verify enough data was read
    if (numrcvd != numbytes){
        QTextStream ss(error_string,QIODevice::WriteOnly);
        ss<<"ServoGetStat "<<addr<<") failed to read chars";
        return false;
    }
    ...
    if (cksum != inbuf[numbytes-1]){
        QTextStream ss(error_string,QIODevice::WriteOnly);
        ss<<"ServoGetStat"<<addr<<":line 103: checksum error";
        return false;
    }
    ...
    if (mod[addr].stat & CKSUM_ERROR){
        QTextStream ss(error_string,QIODevice::WriteOnly);
        ss<<"ServoGetStat: Line 111: Command checksum error!";
        return false;
    }
...
bool ServoSetGain(byte addr, short int kp, short int kd, short int ki,
				  short int il, byte ol, byte cl, short int el,
                  byte sr, byte dc,QString* error_string)
{
SERVOMOD * p;
char cmdstr[16];
...
return NmcSendCmd(addr, SET_GAIN, cmdstr, 14, addr,error_string);
}
...
bool ServoSetGain2(byte addr, short int kp, short int kd, short int ki,
				  short int il, byte ol, byte cl, short int el,
                  byte sr, byte dc, byte sm,QString* error_string)
{
SERVOMOD * p;
char cmdstr[16];
...
return NmcSendCmd(addr, SET_GAIN, cmdstr, 15, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoLoadTraj(byte addr, byte mode, long pos, long vel, long acc, byte pwm,QString* error_string)
{
SERVOMOD * p;
char cmdstr[16];
int count;
...
//                  motor position.                                          //
//---------------------------------------------------------------------------//
void ServoInitPath(byte addr,QString* error_string)
{
SERVOMOD * p;

NmcReadStatus(addr, SEND_POS | SEND_PERROR,error_string);
...
//---------------------------------------------------------------------------//
bool ServoAddPathpoints(byte addr, int npoints, long *path, int freq,QString* error_string)
{
SERVOMOD * p;
char cmdstr[16];
long diff;
...
return NmcSendCmd(addr, ADD_PATHPOINT, cmdstr, (byte)(npoints*2), addr,error_string);
}
...
//                  path point buffer.                                       //
//---------------------------------------------------------------------------//
bool ServoStartPathMode(byte groupaddr, byte groupleader,QString* error_string)
{
    return NmcSendCmd(groupaddr, ADD_PATHPOINT, NULL, 0, groupleader,error_string);
}
...
//                  using ServoLoadTraj().                                   //
//---------------------------------------------------------------------------//
bool ServoStartMove(byte groupaddr, byte groupleader,QString* error_string)
{
return NmcSendCmd(groupaddr, START_MOVE, NULL, 0, groupleader,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoResetPos(byte addr,QString* error_string)
{
...
//---------------------------------------------------------------------------//
bool ServoResetRelHome(byte addr,QString* error_string)
{
byte mode;

mode = REL_HOME;
return NmcSendCmd(addr, RESET_POS, (char *)(&mode), 1, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoSetPos(byte addr, long pos,QString* error_string)
{
char cmdstr[6];
...
return NmcSendCmd(addr, RESET_POS, cmdstr, 5, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoClearBits(byte addr,QString* error_string)
{
return NmcSendCmd(addr, CLEAR_BITS, NULL, 0, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoStopMotor(byte addr, byte mode,QString* error_string)
{
SERVOMOD * p;
...
//---------------------------------------------------------------------------//
bool ServoStopHere(byte addr, byte mode, long pos,QString* error_string)
{
SERVOMOD * p;
char cmdstr[6];
...
return NmcSendCmd(addr, STOP_MOTOR, cmdstr, 5, addr,error_string);
}
...
//---------------------------------------------------------------------------//
bool ServoSetIoCtrl(byte addr, byte mode,QString* error_string)
{
SERVOMOD * p;
...
return NmcSendCmd(addr, IO_CTRL, (char *)(&mode), 1, addr, error_string);
}
...
//                  position.                                                //
//---------------------------------------------------------------------------//
bool ServoSetHoming(byte addr, byte mode,QString* error_string)
{
SERVOMOD * p;
...
//---------------------------------------------------------------------------//
bool ServoHardReset(byte addr, byte mode,QString* error_string)
{
return NmcSendCmd(addr, HARD_RESET, (char *)(&mode), 1, 0,error_string);
}
