...
int SetPathParams2(int freq, int nbuf,
...
if (freq == P_120HZ)    //set fast path mode if using 120 Hz path
  {
  ioctrl = ServoGetIoCtrl(x);
  ServoSetIoCtrl(x, (byte)(ioctrl | FAST_PATH),error_string);
  ioctrl = ServoGetIoCtrl(y);
  ServoSetIoCtrl(y, (byte)(ioctrl | FAST_PATH),error_string);
  if (z)
    {
    ioctrl = ServoGetIoCtrl(z);
    ServoSetIoCtrl(z, (byte)(ioctrl | FAST_PATH),error_string);
    }
  }
else    //clear fast path bit if using slower modes
  {
  ioctrl = ServoGetIoCtrl(x);
  ServoSetIoCtrl(x, (byte)(ioctrl & ~((byte)(FAST_PATH)) ),error_string);
  ioctrl = ServoGetIoCtrl(y);
  ServoSetIoCtrl(y, (byte)(ioctrl & ~((byte)(FAST_PATH)) ),error_string);
  if (z)
    {
    ioctrl = ServoGetIoCtrl(z);
    ServoSetIoCtrl(z, (byte)(ioctrl & ~((byte)(FAST_PATH)) ),error_string);
    }
  }
...
if (freq == P_120HZ)    //set fast path mode if using 120 Hz path
  {
  ServoSetIoCtrl(x, IO1_IN | IO2_IN | FAST_PATH,error_string);
  ServoSetIoCtrl(y, IO1_IN | IO2_IN | FAST_PATH,error_string);
  if (z) ServoSetIoCtrl(z, IO1_IN | IO2_IN | FAST_PATH,error_string);
  }
else     //otherwise, use slow mode
  {
  ServoSetIoCtrl(x, IO1_IN | IO2_IN,error_string);
  ServoSetIoCtrl(y, IO1_IN | IO2_IN,error_string);
  if (z) ServoSetIoCtrl(z, IO1_IN | IO2_IN,error_string);
  }
...
if (!ServoStopMotor(x, ServoGetStopCtrl(x) & (byte)AMP_ENABLE,error_string)) return(0.0);
if (!ServoStopMotor(y, ServoGetStopCtrl(y) & (byte)AMP_ENABLE,error_string)) return(0.0);
if (z)
  if (!ServoStopMotor(z, ServoGetStopCtrl(z) & (byte)AMP_ENABLE,error_string)) return(0.0);

ServoInitPath(x,error_string);   //set the beginning of the path to the current position
ServoInitPath(y,error_string);
if (z) ServoInitPath(z,error_string);
...
int AddPathPoints(QString* error_string)
{
...
while (!at_end)  //GetNextPathpoint() sets the global at_end
  {
  if (!NmcNoOp(x,error_string)) return(-2);                     //read num points from X

  //punt when PIC-SERVO buffer is full
  if ((ServoGetNPoints(x)>bufsize) || (ServoGetNPoints(x)>87))
    {
    if (!NmcNoOp(y,error_string)) return(-2);  //make sure data is updated even if points are not added
    if (!NmcNoOp(z,error_string)) return(-2);
    break;
    }
  ...
  if (!ServoAddPathpoints(x, pcount, xp, pathfreq,error_string)) return(-2);
  if (!ServoAddPathpoints(y, pcount, yp, pathfreq,error_string)) return(-2);
  if (z)
    if (!ServoAddPathpoints(z, pcount, zp, pathfreq,error_string)) return(-2);
  }

if ( !(ServoGetAux(x) & PATH_MODE) )   //start path mode when buffer full
  if (!ServoStartPathMode(group, leader,error_string)) return(-2);
